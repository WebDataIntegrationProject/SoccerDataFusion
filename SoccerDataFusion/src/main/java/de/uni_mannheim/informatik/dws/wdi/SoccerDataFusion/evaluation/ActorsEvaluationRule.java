...
/**
 * {@link EvaluationRule} for the actors of {@link Club}s. The rule simply
 * compares the full set of actors of two {@link Club}s and returns true, in
 * case they are identical.
 * 
 * @author Oliver Lehmberg (oli@dwslab.de)
 * 
 */
public class ActorsEvaluationRule extends EvaluationRule<Club, Attribute> {

	@Override
	public boolean isEqual(Club record1, Club record2, Attribute schemaElement) {
		Set<String> actors1 = new HashSet<>();

		for (Player a : record1.getActors()) {
		...
		for (Player a : record2.getActors()) {
