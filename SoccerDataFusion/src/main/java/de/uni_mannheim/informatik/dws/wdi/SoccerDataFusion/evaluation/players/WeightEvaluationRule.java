...
/**
 * {@link EvaluationRule} for the Weights of {@link Player}s. The rule simply
 * compares the Weight of two {@link Player}s and returns true, in case they
 * are identical.
 * 
 * 
 */
public class WeightEvaluationRule extends EvaluationRule<Player, Attribute> {
	// amount of tolerance -> if values differ within this range, they're considered equal
	...
	public boolean isEqual(Player record1, Player record2, Attribute schemaElement) {
		if(record1.getWeight()== null && record2.getWeight()==null)
			...
			return false;
		else if ((Math.abs(record1.getWeight() - record2.getWeight())) > record1.getWeight() * threshold)  
			return false;
		else 
			return true;
	}
